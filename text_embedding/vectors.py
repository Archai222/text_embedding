...
VECTORFILES = {('CC', 'GloVe', 300): 'glove.840B.300d.txt'}
...
def load(vectorfile, vocabulary=None, dimension=None):
  ...
  '''
    ...
    try:
        ...
        with open(vectorfile, encoding="utf-8") as f:
            n = 0
            for line in f:
                index = line.index(' ')
                word = line[:index]
...
def docs2vecs(documents, f2v=None, weights=None, default=1.0, avg=False, **kwargs):
  ...
  '''
    ...
    if avg:
        return np.vstack(
            sum((f2v.get(feat, z) for feat in document), z) / max(1.0, len(document)) for document in documents)
